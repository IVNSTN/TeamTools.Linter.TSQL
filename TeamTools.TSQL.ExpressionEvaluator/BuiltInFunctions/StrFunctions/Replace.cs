...
namespace TeamTools.TSQL.ExpressionEvaluator.BuiltInFunctions.StrFunctions
{
    public class Replace : SqlGenericFunctionHandler<Replace.ReplaceStrArgs>
    {
        ...
        protected override SqlValue DoEvaluateResultValue(CallSignature<ReplaceStrArgs> call)
        {
            ...
            if (call.ValidatedArgs.Searched.IsPreciseValue
            && call.ValidatedArgs.Replacement.IsPreciseValue)
            {
                if (string.Equals(call.ValidatedArgs.Searched.Value, call.ValidatedArgs.Replacement.Value, StringComparison.OrdinalIgnoreCase))
                {
            ...
            else if (estimatedLengthChange > 0 && resultSize < MaxApproximationLength)
            {
                ...
                resultSize += (int)Math.Ceiling(resultSize * (decimal)estimatedLengthChange / divisor);
