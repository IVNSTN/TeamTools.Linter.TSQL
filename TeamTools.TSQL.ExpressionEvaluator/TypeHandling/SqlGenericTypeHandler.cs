...
namespace TeamTools.TSQL.ExpressionEvaluator.TypeHandling
{
    ...
    where TPreciseValue : IComparable<TPreciseValue>
    {
        ...
        protected SqlGenericTypeHandler(
        ...
        public TValue Convert(SqlValue src)
            => TypeConverter.ImplicitlyConvert<TValue>(src);

        public abstract TSize CombineSize(TSize a, TSize b);

        public TValue ChangeValueTo(TValue old, TPreciseValue newValue, SqlValueSource source)
            => ValueFactory.MakePreciseValue(old.TypeName, newValue, source);

        public TValue ChangeValueTo(TValue old, TSize newSize, SqlValueSource source)
            => ValueFactory.MakeApproximateValue(old.TypeName, newSize, source);
        ...
        protected virtual TSize GetTypeSize(DataTypeReference dataType)
        {
            ...
            return ValueFactory.GetDefaultTypeSize(dataType.GetFullName());
        }
        ...
        protected TValue Compute(
            ...
            Func<TSize, TSize, TSize> approximate)
        {
