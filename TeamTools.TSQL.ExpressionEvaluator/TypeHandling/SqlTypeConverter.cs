    ...
    public class SqlTypeConverter : ISqlTypeConverter
    {
        ...
        where T : SqlValue
        {
            ...
            if (typeof(T) == typeof(SqlStrTypeValue))
            {
                ...
                return (T)ImplicitlyConvertTo(TSqlDomainAttributes.Types.Varchar, from);
            }

            if (typeof(T) == typeof(SqlIntTypeValue))
            {
                return (T)ImplicitlyConvertTo(TSqlDomainAttributes.Types.Int, from);
            }
        ...
        public string EvaluateOutputType(List<SqlValue> values)
        {
            return ExpressionResultTypeEvaluator
                .GetResultingType(values.Select(v => v.TypeName));
        }
