...
namespace TeamTools.TSQL.LinterTests.Routines.ExpressionEvaluator
{
    ...
    where TFunc : SqlFunctionHandler, new()
    {
        ...
        {
            var str = Factory.NewLiteral("VARCHAR", src, default);
            var pos = Factory.NewLiteral("INT", start.ToString(), default);
            var len = Factory.NewLiteral("INT", length.ToString(), default);
        ...
        {
            var str = Factory.NewNull(default);
            var len = Factory.NewLiteral("INT", 123.ToString(), default);

            // str is null
            var res = func.Evaluate(ArgFactory.MakeListOfValues(str, len), Context);
            Assert.That(res?.IsNull, Is.True);

            str = Factory.NewLiteral("VARCHAR", "adsf", default);
