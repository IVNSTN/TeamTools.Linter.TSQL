...
namespace TeamTools.TSQL.ExpressionEvaluatorTests.Tests.TypeHandling.DateTime
{
    ...
    [TestOf(typeof(SqlDateTimeTypeValueFactory))]
    internal class SqlDateTimeTypeValueFactoryTests : BaseSqlTypeHandlerTestClass
    {
        ...
        [Test]
        public void Test_SqlDateTimeTypeValue_DeepCloneCopiesPreciseValue()
        {
            var value = ValueFactory.MakePreciseValue("DATETIME2", new System.DateTime(2005, 07, 31, 12, 30, 55), new SqlValueSource(SqlValueSourceKind.Literal, null));
            Assert.That(value, Is.Not.Null);
            Assert.That(value.Value, Is.EqualTo(new System.DateTime(2005, 07, 31, 12, 30, 55)));

            var clone = value.DeepClone();
            Assert.That(clone, Is.Not.Null);
            Assert.That(clone.IsPreciseValue, Is.True);
            Assert.That(clone.Value, Is.EqualTo(value.Value));
            Assert.That(clone.TypeName, Is.EqualTo("DATETIME2"));
        }

        [Test]
        public void Test_SqlDateTimeTypeValueFactory_MakeMethodsDontFailForUnsupportedType()
        {
