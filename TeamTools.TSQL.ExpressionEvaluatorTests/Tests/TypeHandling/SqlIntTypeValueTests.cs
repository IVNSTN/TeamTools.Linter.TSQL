...
namespace TeamTools.TSQL.LinterTests.Routines.ExpressionEvaluator.TypeHandling
{
    ...
    public sealed class SqlIntTypeValueTests : BaseSqlTypeHandlerTestClass
    {
        ...
        {
            var value = ValueFactory.MakePreciseValue("TINYINT", 123, new SqlValueSource(SqlValueSourceKind.Literal, null));

            Assert.That(value, Is.Not.Null);

            var nextValue = value.ChangeTo(5, new SqlValueSource(SqlValueSourceKind.Variable, null));
            Assert.That(nextValue, Is.Not.Null);
            Assert.That(nextValue.TypeName, Is.EqualTo("TINYINT"));
        ...
        {
            var value = ValueFactory.MakeApproximateValue("SMALLINT", new SqlIntValueRange(-10000, +10000), new SqlValueSource(SqlValueSourceKind.Literal, null));
            ...
            Assert.That(nextValue, Is.Not.Null);
            Assert.That(nextValue.TypeName, Is.EqualTo("SMALLINT"));
            Assert.That(nextValue.SourceKind, Is.EqualTo(SqlValueSourceKind.Variable));
