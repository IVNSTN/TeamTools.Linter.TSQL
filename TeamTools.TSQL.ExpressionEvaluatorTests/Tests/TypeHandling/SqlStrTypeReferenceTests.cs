...
namespace TeamTools.TSQL.LinterTests.Routines.ExpressionEvaluator.TypeHandling
{
    ...
    public sealed class SqlStrTypeReferenceTests
    {
        ...
        {
            // non-unicode type
            Assert.Throws(typeof(ArgumentOutOfRangeException), () => new SqlUnicodeStrTypeReference("VARCHAR", size, factory));
        }
        ...
        public void Test_SqlStrTypeReference_ConstructorFailsOnNullArgs()
        {
            ...
            Assert.Throws(typeof(ArgumentNullException), () => new SqlStrTypeReference("SYSNAME", size, null));
        }

        [Test]
        public void Test_SqlStrTypeReference_ReturnsCorrectWeight()
        {
            var typeRef = new SqlStrTypeReference("VARCHAR", size, factory);
            Assert.That(typeRef.Bytes, Is.EqualTo(size * 1));

            typeRef = new SqlStrTypeReference("CHAR", size, factory);
            Assert.That(typeRef.Bytes, Is.EqualTo(size * 1));
        ...
        {
            var typeRef = new SqlUnicodeStrTypeReference("NVARCHAR", size, factory);
            Assert.That(typeRef.Bytes, Is.EqualTo(size * 2));

            typeRef = new SqlUnicodeStrTypeReference("NCHAR", size, factory);
            Assert.That(typeRef.Bytes, Is.EqualTo(size * 2));
        ...
        public void Test_SqlStrTypeReference_EqualsRespectsSize()
        {
            ...
            var a = new SqlStrTypeReference("VARCHAR", 10, factory);
            var b = new SqlStrTypeReference("VARCHAR", 10, factory);
            ...
            // different type name
            a = new SqlUnicodeStrTypeReference("NCHAR", 20, factory);
            b = new SqlStrTypeReference("VARCHAR", 20, factory);
            ...
            // different size
            a = new SqlStrTypeReference("VARCHAR", 10, factory);
            b = new SqlStrTypeReference("VARCHAR", 20, factory);
            Assert.That(a.Equals(b), Is.False);
            Assert.That(b.Equals(a), Is.False);
        ...
        {
            var def = new SqlStrTypeReference("NVARCHAR", 17, factory);

            Assert.That(def.ToString(), Is.EqualTo("NVARCHAR(17)"));
        }
    }
}
