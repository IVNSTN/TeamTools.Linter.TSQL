...
Правило срабатывает, если осуществляется возврат из нескольких параметров хранимой процедуры в одну и ту же переменную.
...
💡 Возврат из нескольких параметров хранимой процедуры в одну и ту же переменную не имеет смысла и, скорее всего, является логической ошибкой. Пример ошибки:
...
Здесь в одну и ту же переменную `@var` трижды кладется результат из вызова хранимой процедуры: `RETURN` значение и `OUT` из параметров `param_b` и `param_c`. На что рассчитывает автор такого вызова - непонятно.
