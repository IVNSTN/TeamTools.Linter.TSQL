...
<p id="descr">Предикаты фильтрации (WHERE) и объединения (JOIN), которые не могут быть эффективно вычислены и применены к существующим индексам лучше не использовать.</p>
...
💡 Это в том числе:
* Вызовы недетермированных или зависящих от значений колонок таблиц из выборки функций
* Cравнения, основанные на вычислениях с использованием значений полей, на преобразованиях значений полей
* Логическое ИЛИ, примененное к фильтру по полю
* ISNULL, NULLIF, COALESCE, CAST, CONVERT, примененные к полю таблицы
...
* Изменением ISNULLL, NULLIF в OR, который тоже плохой, но может лечиться перекомпиляцией или указаниями OPTIMIZE FOR
