...
namespace TeamTools.TSQL.Linter.Routines.ExpressionEvaluator.BuiltInFunctions.StrFunctions
{
    ...
    public class FormatMessage : SqlGenericFunctionHandler<FormatMessage.FormatArgs>
    {
        ...
        protected override SqlValue DoEvaluateResultValue(CallSignature<FormatArgs> call)
        {
            ...
            var res = BuildFormattedString(groups, wildcards, call.ValidatedArgs.Args, call.Context);
        ...
        private static IList<WildcardElement> GetWildcards(string template)
        {
            return FormatMessageWildcardExtractor.ExtractElements(template)
                .Select(m => new WildcardElement
                {
        ...
        private static BuildResult BuildFormattedString(IList<WildcardElement> groups, IList<WildcardElement> wildcards, IList<SqlValue> args, EvaluationContext context)
        {
            ...
            for (int i = 0; i < wildcards.Count && res.Length < MaxSupportedLength && !varcharMax; i++)
            {
                var wc = wildcards[i];
        ...
        public class FormatArgs
        {
            ...
            public IList<SqlValue> Args { get; } = new List<SqlValue>();
        }
