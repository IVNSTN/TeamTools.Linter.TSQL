...
namespace TeamTools.TSQL.Linter.Routines.ExpressionEvaluator.Evaluation
{
    internal static class EvaluateBuiltInExpressionExtensions
    {
        ...
        public static IList<SqlFunctionArgument> ToArgs(this SqlExpressionEvaluator eval, IList<ScalarExpression> srcParams)
        {
            var output = new List<SqlFunctionArgument>(srcParams?.Count ?? 0);
            if (srcParams is null || srcParams.Count == 0)
            {
                return output;
            }

            foreach (var arg in srcParams)
            {
                output.Add(eval.ToArg(arg));
            }

            return output;
        }

        public static IList<SqlFunctionArgument> ToArgs(this SqlExpressionEvaluator eval, params SqlFunctionArgument[] srcParams) => srcParams.ToList();
        ...
        public static SqlFunctionArgument ToArg(this SqlExpressionEvaluator eval, ColumnReferenceExpression datePart)
        {
            ...
            || !DatePartConverter.SupportedDateParts.Contains(datePartNameCandidate))
            {
