...
namespace TeamTools.TSQL.Linter.Routines.ExpressionEvaluator.TypeHandling
{
    ...
    {
        private static readonly string IntFallbackTypeName = "dbo.BIGINT";
        private static readonly IDictionary<string, SqlBigIntValueRange> TypeRanges = new Dictionary<string, SqlBigIntValueRange>(StringComparer.OrdinalIgnoreCase);

        static SqlBigIntTypeValueFactory()
        {
            // TODO : fix range
            TypeRanges.Add("dbo.BIGINT", new SqlBigIntValueRange(int.MinValue, int.MaxValue));
        }
